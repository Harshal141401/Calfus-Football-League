...
const { resultFromScore, scorePrediction } = require("./scoring");

async function settleFixture(fixtureId, homeScore, awayScore) {
  ...
  const result = resultFromScore(h, a);
  await fixtures.updateOne(
    { _id: fixtureId },
    { $set: { homeScore: h, awayScore: a, result, status: "settled", settledAt: new Date() } }
  );
  ...
  for (const p of preds) {
    const s = scorePrediction(p, { homeScore: h, awayScore: a });
...
async function unsettleFixture(fixtureId) {
  await collections.fixtures().updateOne(
    { _id: fixtureId },
    { $set: { status: "scheduled" }, $unset: { homeScore: "", awayScore: "", result: "", settledAt: "" } }
  );
